# VinylDigger Implementation Plan
...
**Architecture:** A SwiftUI app shell over `VinylDiggerKit`, a Swift package holding all logic. `GraphEngine` is pure (no I/O), `DiscogsAPI` speaks only HTTP, `Store` only persists, `Outbox` retries failed writes. Playback runs through a hidden `WKWebView` driving the YouTube IFrame Player API.
...
- Bundle identifier: `de.schakal.VinylDigger`. Project root: `~/Developer/Digger`.
- Database path: `~/Library/Application Support/VinylDigger/vinyldigger.sqlite`.
- API cache path: `~/Library/Caches/de.schakal.VinylDigger/`.
...
- Tests never touch the live network. All `DiscogsAPI` tests run against fixtures in `VinylDiggerKit/Tests/VinylDiggerKitTests/Fixtures/`.
...
| File | Responsibility |
|---|---|
| `project.yml` | XcodeGen definition for the `VinylDigger` macOS app target |
| `VinylDiggerKit/Package.swift` | Package manifest, GRDB dependency, test resources |
| `VinylDiggerKit/Sources/VinylDiggerKit/Secrets/SecretStore.swift` | Keychain protocol + implementation for the token |
| `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/DiscogsModels.swift` | Codable models for every consumed endpoint |
| `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/RateLimiter.swift` | Token bucket with injectable clock |
| `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/HTTPTransport.swift` | Transport protocol + URLSession implementation |
| `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/DiscogsClient.swift` | Endpoint methods, auth header, retry on 429 |
| `VinylDiggerKit/Sources/VinylDiggerKit/Store/Records.swift` | GRDB record types |
| `VinylDiggerKit/Sources/VinylDiggerKit/Store/AppDatabase.swift` | Migrations, queries, write helpers |
| `VinylDiggerKit/Sources/VinylDiggerKit/Graph/GraphTypes.swift` | `NodeKind`, `EdgeKind`, `GraphNode`, `GraphEdge`, `DecisionKind` |
| `VinylDiggerKit/Sources/VinylDiggerKit/Graph/WeightPropagator.swift` | Seed-to-node weight propagation |
| `VinylDiggerKit/Sources/VinylDiggerKit/Graph/Scorer.swift` | Release scoring |
| `VinylDiggerKit/Sources/VinylDiggerKit/Graph/QueuePlanner.swift` | Ordering + label diversity cap |
| `VinylDiggerKit/Sources/VinylDiggerKit/Outbox/OutboxProcessor.swift` | Pending Discogs writes, backoff, attempt cap |
| `VinylDiggerKit/Sources/VinylDiggerKit/Bootstrap/BootstrapImporter.swift` | One-time import of the manual research dumps |
| `VinylDiggerKit/Sources/VinylDiggerKit/Session/QueueService.swift` | Ties API + Store + Graph together, lazy hydration |
| `App/VinylDiggerApp.swift` | App entry point, dependency wiring |
...
**Files:**
- Create: `project.yml`
- Create: `VinylDiggerKit/Package.swift`
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/VinylDiggerKit.swift`
- Create: `App/Info.plist`
- Create: `App/VinylDiggerApp.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/ScaffoldingTests.swift`

**Interfaces:**
- Consumes: nothing.
- Produces: a buildable `VinylDiggerKit` package and a runnable `VinylDigger.app`. Later tasks add sources under `VinylDiggerKit/Sources/VinylDiggerKit/<Area>/`.

- [ ] **Step 1: Write the failing test**

Create `VinylDiggerKit/Tests/VinylDiggerKitTests/ScaffoldingTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test`
Expected: FAIL — no such module `VinylDiggerKit` (the package does not exist yet).
...
targets:
  VinylDigger:
    ...
    sources:
      - App
    dependencies:
      - package: VinylDiggerKit
    info:
      path: App/Info.plist
      properties:
        CFBundleDisplayName: VinylDigger
...
The app is deliberately **not** sandboxed. Sandboxing would redirect `~/Library/Application Support/VinylDigger/` into a container path, contradicting the spec, and this is a local personal tool that is never distributed through the App Store.
...
struct VinylDiggerApp: App {
    var body: some Scene {
        WindowGroup {
            Text("VinylDigger \(VinylDiggerKit.version)")
...
```bash
cd ~/Developer/Digger
git add project.yml App VinylDiggerKit
git commit -m "feat: scaffold VinylDiggerKit package and macOS app target"
```
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Secrets/SecretStore.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/SecretStoreTests.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/SecretStoreTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter SecretStoreTests`
...
```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit/Secrets VinylDiggerKit/Tests/VinylDiggerKitTests/SecretStoreTests.swift
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/DiscogsModels.swift`
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/RateLimiter.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/DiscogsModelsTests.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/RateLimiterTests.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/FixtureLoader.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/FixtureLoader.swift`:
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/DiscogsModelsTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter DiscogsModelsTests`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/RateLimiterTests.swift`:
...
final class RateLimiterTests: XCTestCase {
    ...
    func testConsecutiveDeficitCallsStackRatherThanRepeat() async {
        ...
        // No clock advance between these two — the debt must accumulate, or both
        // callers would wake at the same instant and defeat the limiter.
        let first = await limiter.reserve()
        let second = await limiter.reserve()

        XCTAssertEqual(first, 1.0, accuracy: 0.0001)
        XCTAssertEqual(second, 2.0, accuracy: 0.0001)
    }
}
```
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter RateLimiterTests`
...
```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI VinylDiggerKit/Tests/VinylDiggerKitTests
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/HTTPTransport.swift`
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/DiscogsAPI/DiscogsClient.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/DiscogsClientTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter DiscogsClientTests`
...
public actor DiscogsClient {
    ...
    /// Hoisted out of `fetchPage` — Swift does not allow a generic type to be
    /// declared inside a generic function.
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Store/Records.swift`
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Store/AppDatabase.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/AppDatabaseTests.swift`:
...
final class AppDatabaseTests: XCTestCase {
    ...
    func testMigrationsCreateAllTables() throws {
        ...
        let tables = try db.read { database in
            // AUTOINCREMENT makes SQLite create its own sqlite_sequence table, so the
            // filter has to exclude SQLite internals as well as GRDB's bookkeeping.
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter AppDatabaseTests`
Expected: FAIL — cannot find `AppDatabase` in scope.

- [ ] **Step 3: Create the shared graph enums**

Create `VinylDiggerKit/Sources/VinylDiggerKit/Graph/GraphTypes.swift`:
...
```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit/Store VinylDiggerKit/Sources/VinylDiggerKit/Graph/GraphTypes.swift VinylDiggerKit/Tests/VinylDiggerKitTests/AppDatabaseTests.swift
...
**Files:**
- Modify: `VinylDiggerKit/Sources/VinylDiggerKit/Graph/GraphTypes.swift`
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Graph/WeightPropagator.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/WeightPropagatorTests.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/WeightPropagatorTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter WeightPropagatorTests`
Expected: FAIL — cannot find `NodeID` in scope.

- [ ] **Step 3: Extend the graph types**

Append to `VinylDiggerKit/Sources/VinylDiggerKit/Graph/GraphTypes.swift`:
...
```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit/Graph VinylDiggerKit/Tests/VinylDiggerKitTests/WeightPropagatorTests.swift
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Graph/Scorer.swift`
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Graph/QueuePlanner.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/ScorerTests.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/QueuePlannerTests.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/ScorerTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter ScorerTests`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/QueuePlannerTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter QueuePlannerTests`
...
```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit/Graph VinylDiggerKit/Tests/VinylDiggerKitTests/ScorerTests.swift VinylDiggerKit/Tests/VinylDiggerKitTests/QueuePlannerTests.swift
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Outbox/OutboxProcessor.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/OutboxProcessorTests.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/OutboxProcessorTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter OutboxProcessorTests`
...
```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit/Outbox VinylDiggerKit/Tests/VinylDiggerKitTests/OutboxProcessorTests.swift
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Bootstrap/BootstrapImporter.swift`
- Test: `VinylDiggerKit/Tests/VinylDiggerKitTests/BootstrapImporterTests.swift`
...
The two dumps already sit in `VinylDiggerKit/Tests/VinylDiggerKitTests/Fixtures/`. `dig_profile.json` has the shape `{"profile": {"<seed name>": {"id": Int, "discogs_name": String, "aliases": [String], "groups": [String], "top_labels": [[String, Int]]}}, "ranked_labels": [[String, Int, [String]]]}`. `dig_labels.json` maps a label name to `{"id": Int, "picks": [{"id": Int, "catno": String?, "artist": String, "title": String, "year": Int?, "want": Int, "have": Int, "styles": [String], "videos": [String]}]}`.
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/BootstrapImporterTests.swift`:
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter BootstrapImporterTests`
...
Note: with an exact `artistName = ?` match this test does fail — no pick credits "Carl A. Finlow" alone; `20:20 Vision` lists him as `Carl A. Finlow, Ralph Lawson & Domenic Capello*`. The Step 3 code above therefore already carries the widened `OR artistName LIKE ?` match. Of the four seeds, Kelvin K and Jamie Anderson match exactly, Finlow only via LIKE, and Tyler Stadius has no pick at all in the dumps — so the bootstrap yields three artist-to-label edges.

- [ ] **Step 5: Commit**

```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit/Bootstrap VinylDiggerKit/Tests/VinylDiggerKitTests/BootstrapImporterTests.swift
...
**Files:**
- Create: `VinylDiggerKit/Sources/VinylDiggerKit/Session/QueueService.swift`
...
Create `VinylDiggerKit/Tests/VinylDiggerKitTests/QueueServiceTests.swift`:
...
final class QueueServiceTests: XCTestCase {
    ...
    // `decide` drains the outbox straight away, so a successful write leaves no
    // queued row. The two tests below split the delivered and the failed path.
...
Run: `cd ~/Developer/Digger/VinylDiggerKit && swift test --filter QueueServiceTests`
Expected: FAIL — cannot find `QueueService` in scope.

- [ ] **Step 3: Add the `owned` column**

Add a `v2` migration inside `Self.migrator` in `VinylDiggerKit/Sources/VinylDiggerKit/Store/AppDatabase.swift`, directly after the `registerMigration("v1")` block:
...
Add the matching property to `ReleaseRecord` in `VinylDiggerKit/Sources/VinylDiggerKit/Store/Records.swift` — a stored property after `hydrated`, and a new parameter at the end of the initialiser:
...
```bash
cd ~/Developer/Digger
git add VinylDiggerKit/Sources/VinylDiggerKit VinylDiggerKit/Tests/VinylDiggerKitTests/QueueServiceTests.swift
...
- Modify: `App/VinylDiggerApp.swift`

**Interfaces:**
- Consumes: nothing from `VinylDiggerKit`.
- Produces:
...
Replace the body of `App/VinylDiggerApp.swift`:
...
If nothing plays, open Safari's Develop menu → the VinylDigger process → the web inspector, and check the console inside the page.
...
cp VinylDiggerKit/Tests/VinylDiggerKitTests/Fixtures/dig_profile.json App/Resources/
cp VinylDiggerKit/Tests/VinylDiggerKitTests/Fixtures/dig_labels.json App/Resources/
```

No `project.yml` change is needed: the existing `sources: - App` entry already picks the dumps up and xcodegen files them as resources, so they land flat in `Contents/Resources` — exactly where `Bundle.main.url(forResource:withExtension:)` looks. Adding a second `path: App/Resources` folder reference on top of that ships both dumps twice (once flat, once under `Resources/Resources`).
...
Replace `App/VinylDiggerApp.swift`:
...
The Settings scene and the auxiliary windows arrive in Task 13 together with the views they host. This task must build and launch on its own — do not reference `SettingsView` here.
...
Without a token in the Keychain the app launches and the status line reads "Token fehlt — in den Einstellungen eintragen". That is the correct behaviour at this point; Settings does not exist yet.
